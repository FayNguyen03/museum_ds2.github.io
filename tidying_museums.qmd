...
```{r}
museums <- museums |> 
  filter(DISCIPLINE != "ZAW", DISCIPLINE != "BOT", !str_detect(COMMONNAME,".SOCIETY$")) 
```
...
```{r}
write_csv(uscb_demo_1, "Data/population_age.csv")
```


```{r}
ucsb_demo_2 <- read_csv("Data/raw_data/UCSB_income_2022.csv") |>
  select(- Geography) |>
  rename(zip_code = 'Geographic Area Name') |>
  mutate(zip_code = str_replace(zip_code,'ZCTA5 ',''))
```

We extracted this population dataset into a clean data set for use named `income.csv`.

```{r}
write_csv(ucsb_demo_2, "Data/income.csv")
```


These two files above are in the area units that classifiied by zip code. We also extracted a data set of population in the city units.
