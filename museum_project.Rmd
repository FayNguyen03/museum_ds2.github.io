...
We mainly focus on the dataset from [IMLS](https://www.imls.gov/research-evaluation/data-collection/museum-data-files). 
...
moran.mc(counties_sf$n, counties_nbw, nsim = 999) # Loop
...
popup_content <- function(name, rate, open_hour,web) {
  popupContent <- mapply(function(name_val, rate_val, open_hour_val, web) {
    paste0(
      "<h2>", name_val, "</h2>",
      "<h4>", 
      "<p><strong>Rate (TripAdvisor):</strong> ", rate_val, "</p>",
      "<p><strong>Open Hour:</strong> ", open_hour_val, "</p>",
      "<a href=\"",web,"\">Link</a>"
    )
  }, name, rate, open_hour, web)
...
map <- leaflet(data = minneseums_sf) %>%
  ...
  addCircleMarkers(
    ...
    popup = ~popup_content(name, rate, operation, website_1)
  )
