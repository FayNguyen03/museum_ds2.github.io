...
We mainly focus on the dataset from [IMLS](https://www.imls.gov/research-evaluation/data-collection/museum-data-files). There are 7 categories of museums regulated by the IMLS, represented by the `DISCIPLINE` with each code corresponding to a specific category.
...
inputPanel(
  dynamicInput <- renderUI({
    ...
    if (input$unit == "State") {
      ...
      selectInput("aamreg", "AAMREG", choices = areg , selected = 1)
    } 
    else if (input$unit == "BEAREG") {
      selectInput("beareg", "BEAREG", choices = breg, selected = 2)
    } 
...
Moran's I of counties Point-pattern analysis density analysis household_income population_change zip codes
...
popup_content <- function(name, rate, operation, website_1, address, city) {
  ...
  open_hour_val <- ifelse(operation != FALSE, paste0("<p><strong>Open Hour:</strong> ", operation, "</p>"), "")
  web_val <- ifelse(website_1 != FALSE, paste0("<a href=\"", website_1, "\">Link</a>"), "")
  
  popupContent <- mapply(function(name_val, rate_val, open_hour_val, web_val, address_val, city_val) {
    paste0(
      "<h2>", name_val, "</h2>",
  ...
  }, name, rate_val, open_hour_val, web_val, address, city)
...
map <- leaflet(data = minneseums_sf) %>%
  ...
  addCircleMarkers(
    ...
    popup = ~popup_content(name, rate, operation, website_1, address, city)
  )
...
- TripAdvisor API: Due to the limitation in the number of API calls, we can only fetch the data of the museums in Minnesota. 